...
function [results] = lexical2(subject) % eddxddp. call command window
...
try
    ...
    drctpath = 'C:\Users\PychComp\Desktop\exp4c'; 
    addpath (drctpath) 

    % Trial BLOCKS

    [~,~,deneme] = xlsread('trial.xlsx');
    trials = deneme (:,1); % stimuli (totally 8 stimuli)
    ...
    [~,~,VorNonV1] = xlsread('VerbNonVerb1.xlsx');
    ...
    [~,~,VorNonV2] = xlsread('VerbNonVerb2.xlsx');
    ...
    DrawFormattedText(Mywindow, ['Hoşgeldiniz\n\n Bu çalışma iki kısımdan oluşmaktadır. \n\n Her iki kısımda da aşağıda verilen görevi yerine getirmeniz beklenmektedir. ' ...
        '\n\n Görev: \n\n   ' ...
        'Her bir sabitleme noktasından sonra ekranda anlamlı ve anlamsız kelimeler belirecektir. \n\n' ...
        'Mümkün olduğunca hızlı ve doğru bir şekilde ekranda beliren her bir sözcüğün kelime olup olmadığına karar vermelisiniz.    ' ...
        ...
        try
            ...
            while time2 < 100000
                ...
                if (keyIsDown) % has a key been pressed ?
                    ...
                    if condition22 {i} == 1 && strcmp (response_key2{i}, 'd') == 1  % verb and bottom key (d)
                        response_correct2(i) = 0;  % it's error


                    elseif condition22 {i} == 1 && strcmp (response_key2{i}, 'f') == 1  % verb and up key (f)
                        response_correct2(i) = 1;  % it's correct

                    elseif condition22 {i} == 2 && strcmp (response_key2{i}, 'd') == 1  % nonverb and bottom key (d)
                        response_correct2(i) = 1; % it's correct


                    elseif condition22 {i} == 2 && strcmp (response_key2{i}, 'f') == 1  % nonverb and up key (f)
                        response_correct2(i) = 0;  % it's error
  ...
  % ---- Finished Incongruent Condition----------------------------- 
        ...
        try
            ...
            while time1 < 100000
                ...
                if (keyIsDown) % has a key been pressed ?
                    ...
                    if condition12 {i} == 1 && strcmp (response_key1{i}, 'd') == 1  % verb and bottom key (d)
                        response_correct1(i) = 0;   % it's error


                    elseif condition12 {i} == 1 && strcmp (response_key1{i}, 'f') == 1  % verb and up key (f)
                        response_correct1(i) = 1;  % it's correct

                    elseif condition12 {i} == 2 && strcmp (response_key1{i}, 'd') == 1  % nonverb and bottom key (d)
                        response_correct1(i) = 1;  % it's correct


                    elseif condition12 {i} == 2 && strcmp (response_key1{i}, 'f') == 1  % nonverb and up key (d)
                        response_correct1(i) = 0;  % it's error
    ...
    cd([drctpath '\results2']);

    df = sprintf('lexical2_Subject%d.xlsx', subject);
    fout=sprintf('lexical2_Subject%d.mat', subject);
    save(fout, 'results');
   
    writetable(results.T2,df,'Sheet',1);
    writetable(results.T1,df,'Sheet',2);
